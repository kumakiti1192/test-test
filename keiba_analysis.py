...
def safe_driver_get_multi(
    ...
    url: str,
    *,
    wait_xpaths: Tuple[str, ...],
    timeout: int = 45,
    max_retry: int = 3,
) -> bool:
    cond = EC.any_of(
        *[EC.presence_of_element_located((By.XPATH, xp)) for xp in wait_xpaths]
    )
    for att in range(1, max_retry + 1):
        try:
            driver.set_page_load_timeout(timeout)
...
def process_one_date(date: str, files: List[Path]) -> pd.DataFrame:
    ...
    for (place, r), grp in preds.groupby(["場所", "R"]):
        ...
        result = get_race_results_with_odds(rid)
